...
- [x] Share: native share sheet + share link (`POST .../share`) — editor "Share" generates a 7-day link and opens the OS share sheet (WhatsApp etc.). **Still to do:** render the painted result to an image + save-to-gallery (needs view-shot + media-library).
