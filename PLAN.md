...
| Billing | `POST /api/billing/subscriptions`, `GET /api/billing/subscriptions/current` (`billing/`) — Razorpay. `GET /api/billing/project-credit/options` prices one more project (with and without a plan), a reopen, and counts paid-for projects not yet created. **Paying is a web Checkout flow — the app reads the price and links out** |
...
- [x] Segmentation flow: trigger `POST .../segment`, poll status, handle failure — editor offers **AUTO** (AI, spends an auto-mask credit) and **MANUAL** (free on every tier), polls `GET /status` every 2 s while `SEGMENTING`, surfaces `FAILED` + reason with retry, and turns each coded 402 into an action: `AUTO_MASK_UNAVAILABLE` → mark walls by hand, `ASK_RETAILER` → ask the shop, `SUBSCRIPTION_REQUIRED` / `IMAGE_LIMIT_REACHED` → say what ran out. **Tap-to-mark walls is wired** (`POST .../segment/point`, mapped through the canvas's `cover` fit), and hand-marked walls can be deleted.
